...
@pytest.mark.parametrize("league, team, week, box_scores, results",
                         [
                            ...
                            (league_2022, team_2022_0, 1, None, {
                                "len" : 19,
                                "player_names" : ['Najee Harris', 'Travis Kelce', 'Keenan Allen', 'Travis Etienne Jr.', 'Breece Hall', 'Courtland Sutton', 'Christian Kirk', 'Russell Wilson', 'DeVonta Smith', 'Cordarrelle Patterson', 'Allen Lazard', 'Matthew Stafford', 'George Pickens', 'Alexander Mattison', 'Brian Robinson', 'Isaiah McKenzie', 'Tyler Bass', 'Browns D/ST', 'Jerick McKinnon']
                            }),
...
def test_get_lineup(league: League, team: Team, week: int, box_scores, results: dict):
    if league.year < 2019:
        with pytest.raises(Exception) as e_info:
            assert utils.get_lineup(league, team, week, box_scores), Exception("Cant use box score before 2019")
    else:
...
@pytest.mark.parametrize("lineup, slot, n, results",
                         [
                            ...
                            (lineup_2022_t0_w1, 'WR', 3, ["Christian Kirk", "Courtland Sutton", "Isaiah McKenzie"]),
                            (lineup_2022_t0_w1, 'TE', 2, ["Travis Kelce"]), # Select top 2, but there is only 1 TE
...
@pytest.mark.parametrize("league, week, func, box_scores, slot, result",
                         [
                            (league_2022, 1, utils.get_best_lineup, None, None, [6, 8, 10, 1, 4, 5, 2, 7, 3, 9]),
                            (league_2022, 4, utils.get_best_trio, None, None, [5, 7, 3, 1, 2, 10, 9, 8, 4, 6]),
                            (league_2022, 4, utils.get_lineup_efficiency, box_scores_2022_4, None, [6, 10, 1, 3, 9, 4, 7, 2, 5, 8]),
                            (league_2022, 1, utils.avg_slot_score, None, 'QB', [8, 5, 1, 6, 2, 4, 10, 3, 7, 9]),
                            (league_2022, 1, utils.avg_slot_score, None, 'RB', [2, 1, 6, 7, 9, 10, 5, 8, 4, 3]),
                            (league_2022, 1, utils.sum_bench_points, None, None, [8, 7, 1, 6, 2, 5, 3, 10, 4, 9]),
                         ])
def test_sort_lineups_by_func(league: League, week: int, func: Callable, box_scores: list, slot: str, result: int):
    if 'slot' in inspect.getfullargspec(func).args:
        sorted_teams = utils.sort_lineups_by_func(league, week, func, box_scores, slot=slot)
    else:
