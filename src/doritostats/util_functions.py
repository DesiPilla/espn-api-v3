...
def set_league_endpoint(league: League):
    ...
    if (league.year >= pd.datetime.today().year):
...
def get_roster_settings(league: League):
    ...
    endpoint = '{}&view=mMatchupScore&view=mTeam&view=mSettings'.format(
        league.endpoint, league.year, league.league_id)
...
def print_current_standings(league: League):
    ...
    for team in standings:
        results_table += [[team.team_name, team.wins,
                           team.losses, team.ties, team.points_for, team.owner]]
    print('\nWeek {}\n'.format(league.currentMatchupPeriod),
          table(results_table,
                headers=['Team', 'Wins', 'Losses',
                         'Ties', 'Points Scored', 'Owner'],
                floatfmt='.2f'))
