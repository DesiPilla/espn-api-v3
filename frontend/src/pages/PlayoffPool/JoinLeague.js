...
const JoinLeague = () => {
  ...
  const handleCreateTeam = async () => {
    if (!user || !newTeamName.trim()) {
      alert(newTeamName.trim() ? 'Please log in first' : 'Please enter a team name');
      return;
    }
    ...
    try {
      ...
      const result = await playoffPoolAPI.joinLeague(leagueId, newTeamName.trim());
  ...
  const handleConfirmMultiple = async () => {
    try {
      ...
      if (confirmationNeeded.action === 'claim') {
        await playoffPoolAPI.claimTeam(leagueId, confirmationNeeded.teamId, true);
      } else if (confirmationNeeded.action === 'create') {
        await playoffPoolAPI.joinLeague(leagueId, confirmationNeeded.teamName, true);
      }
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div>
                <h1 style={{ fontSize: '28px', fontWeight: '700', color: '#1f2937', margin: 0, marginBottom: '8px' }}>
                  Join League
                </h1>
            ...
            {/* League ID Input */}
            <div style={{ padding: '24px', borderBottom: '1px solid #f1f5f9', backgroundColor: '#fafbfc' }}>
              ...
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    cursor: 'pointer',
                    transition: 'all 0.15s',
                    opacity: (!leagueId.trim() || loading) ? 0.5 : 1
                  }}
                  onMouseEnter={e => {
            ...
            <div style={{
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', gap: '16px', alignItems: 'center', fontSize: '14px', color: '#6b7280' }}>
                  <span>Created by: {leagueInfo.created_by}</span>
                  <span>Teams: {leagueInfo.claimed_teams + leagueInfo.unclaimed_teams} / {leagueInfo.num_teams}</span>
                  <span>Available: {leagueInfo.available_spots}</span>
                </div>
              </div>

              {(leagueInfo.unclaimed_teams > 0 || leagueInfo.available_spots > 0) ? (
                <>
                  {/* Table Headers */}
                  <div style={{ 
                    display: 'grid', 
                  ...
                  <div style={{ borderTop: '1px solid #f1f5f9' }}>
                    {(() => {
                      ...
                      if (leagueInfo.available_spots > 0) {
                        allSlots.push({
                          type: 'empty',
                      ...
                      return allSlots.map((slot) => (
                        <div key={slot.type === 'team' ? slot.data.id : 'empty-new'} 
                             style={{ 
                               display: 'grid', 
                               gridTemplateColumns: '8% 8% 25% 25% 22% 12%',
                               padding: '12px 0',
                          ...
                          <div style={{ textAlign: 'center', padding: '0 8px' }}>
                            {slot.type === 'team' ? (
                              <div style={{
                                width: '32px',
                                height: '32px',
                                background: slot.data.user ? 'linear-gradient(135deg, #3b82f6, #8b5cf6)' : '#f1f5f9',
                                borderRadius: '50%',
                                display: 'flex',
                                alignItems: 'center',
                                ...
                                <span style={{ 
                                  color: slot.data.user ? 'white' : '#64748b', 
                                  fontSize: '12px', 
                                  fontWeight: 'bold' 
                                }}>
                          ...
                          <div style={{ textAlign: 'center', padding: '0 8px' }}>
                            <div style={{ display: 'flex', gap: '4px', justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap' }}>
                              {slot.type === 'team' ? (
                                !slot.data.user ? (
                                  <button
                                    onClick={() => handleClaimTeam(slot.data.id)}
                                ...
                                creatingNewTeam ? (
                                  <div style={{ display: 'flex', gap: '2px' }}>
                                    <button
                                      ...
                                      disabled={!newTeamName.trim() || joinLoading}
                                      style={{
                                        padding: '4px 8px',
                                        border: '1px solid #34d399',
                                        borderRadius: '4px',
                                        ...
                                        cursor: 'pointer',
                                        transition: 'all 0.15s',
                                        opacity: (!newTeamName.trim() || joinLoading) ? 0.5 : 1
                                      }}
                                    >
                                      ✓
                                    </button>
                ...
                <div style={{ padding: '48px 24px', textAlign: 'center', color: '#9ca3af' }}>
                  <p style={{ fontSize: '18px', marginBottom: '16px', fontWeight: '500' }}>League Full</p>
                  <p style={{ fontSize: '14px', marginBottom: '0' }}>
                    This league is full and has no available spots.
                  </p>
                </div>
              )}
            </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                ...
                >
                  {joinLoading ? 'Joining...' : `Yes, create ${confirmationNeeded.ordinal} team`}
                </button>
              </div>
            </div>
          </div>
