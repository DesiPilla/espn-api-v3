...
const ReturningLeagueSelector = ({ dropdownClassName }) => {
    ...
    useEffect(() => {
        fetchWithRetry("/api/distinct-leagues-previous/", {}, maxRetries)
            ...
            .catch((error) =>
                console.error("Error fetching distinct leagues:", error)
            );
    }, []);
    ...
    return (
        <>
            <h3>
                Returning for the new season? Select your old league and easily
                add it for the 2025-26 season.
            </h3>
            <div className="select" style={{ display: "inline-block" }}>
                ...
                >
                    <option value="">
                        {" "}
                        - Select your historical league -{" "}
                    </option>
                    {leaguesPreviousDistinct.map((league) => (
                        <option key={league.league_id} value={league.league_id}>
                            {league.league_year} - {league.league_name} (
                            {league.league_id})
                        </option>
                    ))}
                </select>
