...
const ScoringSettingsEditor = ({ leagueId, onClose, onSave }) => {
    ...
    const loadScoringSettings = async () => {
        try {
            setLoading(true);
            const data = await playoffPoolAPI.getLeagueScoringSettings(leagueId);
            ...
            const flatSettings = {};
            Object.values(data.scoring_settings || {}).forEach(categoryStats => {
    ...
    const handleSave = async () => {
        try {
            ...
            const updates = Object.entries(scoringSettings)
                .filter(([statName, value]) => value !== originalSettings[statName])
                .map(([statName, value]) => ({
                    stat_name: statName,
                    multiplier: value
                }));
            ...
            if (updates.length > 0) {
                await playoffPoolAPI.updateLeagueScoringSettings(leagueId, updates);
    ...
    const hasChanges = JSON.stringify(scoringSettings) !== JSON.stringify(originalSettings);
                                        ...
                                        {stats.map((stat) => (
                                            <div key={stat.stat_name} style={{
                                                ...
                                                <input
                                                    type="number"
                                                    step="0.01"
                                                    value={scoringSettings[stat.stat_name] || 0}
                                                    onChange={(e) => handleSettingChange(stat.stat_name, e.target.value)}
                                                    style={{
                                                        width: '80px',
